...
void CGLRenderer::DrawScene(CDC *pDC)
{
	...
	mat_amb[0] = 0; mat_amb[1] = 0.5; mat_amb[2] = 0.5;
	...
	mat_amb[0] = mat_amb[1] = 0; mat_amb[2] = 0.5;
	mat_dif[0] = mat_dif[1] = 0; mat_dif[2] = 1;
	mat_spec[0] = mat_spec[1] = mat_spec[2] = 0.2;
	mat_shin = 112.0;
	...
	if (blueOn)
	{
		mat_dif[0] = 0.0; mat_dif[1] = 0.2; mat_dif[2] = 0.2;
		mat_amb[0] = mat_amb[1] = mat_amb[2] = 0.0;
		...
		GLfloat light3_ambient[] = { 1, 0,0, 1.0 };
		GLfloat light3_diffuse[] = { 1.0, 0, 1, 1.0 };
		GLfloat light3_specular[] = { 1, 1, 1.0, 1.0 };
	...
	DrawConePart(32, M_PI / 2, upperPtYDist, radius);
	...
	glTranslatef(0, 2 * upperPtYDist+0.09, 0);
	radius -= upperPtXDist;
	DrawConePart(32, M_PI / 2, upperPtYDist, radius);
...
void CGLRenderer::DrawHalfSphere(int horizontalPrecision, int verticalPrecision, double radius, double* color)
{
	...
	double yAngleStep = (M_PI / 2) / (verticalPrecision - 1);
	double yAngle = M_PI / 2 - yAngleStep;
	double xAngleStep = (2 * M_PI) / horizontalPrecision;
	double xAngle = 0;
...
void CGLRenderer::DrawSphere(int horizontalPrecision, int verticalHalfPrecision, double radius, double* color)
{
...
void CGLRenderer::DrawPrismC(int numOfSides, double height, double radius, double* color)
{
	...
	double angleStep = (2 * M_PI) / numOfSides;
	double xNor, zNor;
...
void CGLRenderer::DrawPrism(int numOfSides, double height, double radius, double* color)
{
	...
	normalAngle = (M_PI / 2) - ((M_PI - angleStep) / 2);
...
void CGLRenderer::DrawConePart(int precision, double angle, double partLen, double radius)
{
	...
	double currentAngle = 0;
	double angleStep = (2 * M_PI) / precision;
...
void CGLRenderer::DrawWall(double height, double width, double heightPrecision, double widthPrecision)
{
...
void CGLRenderer::DrawWallPart(double quadHeight, double quadWidth, double quadNum, double startingHeight)
{
...
void CGLRenderer::DrawTry(int numOfSides, double height, double radius, double* color)
{
	int arraySize = numOfSides * 2 * 3;
	double* vertices = new double[arraySize];
	double* colors = new double[arraySize];
